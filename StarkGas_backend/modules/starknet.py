...
class Starknet:
    ...
    async def get_estimate(self, transaction):
        estimate = await self.account.client.estimate_fee(transaction)
        fee_w = estimate.gas_price * estimate.gas_usage
        fee_h = round(fee_w / 10 ** 18, self.ROUND)
